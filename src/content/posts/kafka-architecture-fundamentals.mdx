...
제약 1: In-Memory Cache 사용
  ...
  <- Broker의 메모리와 보존 정책에 의해 제한됨
  <- Broker 재시작 후: 새로운 Epoch 할당, 이전 식별 정보 소실 -> 중복 가능성
  <- 해결: acks=all 설정 필수 (복제 완료 후 ACK)

제약 2: Broker 간 복제 미포함
  acks=1 설정 시:
  - Leader Broker A에만 저장됨
  - ACK 반환 (Follower 복제 전)
  - Leader A 실패 -> Follower B 선출 전에 재전송
  - Broker B는 메시지를 모름 -> 중복 가능성
...
활성화 (Kafka 3.0+):
  처리량: 거의 동일 (환경에 따라 1-5% 내외 감소)
  지연시간: 2ms (변화 없음)
  
이유: 
  - (ProducerId, Sequence) 체크 오버헤드 (매우 낮음)
  - 메모리 캐시 관리 (최적화됨)
  - 성능 영향 무시할 수 있는 수준 -> 권장
```
...
**주의:** 본 섹션은 **Kafka 2.x ~ 3.x의 Classic Consumer Group Protocol**을 설명합니다. 
Kafka 4.0+에서는 새로운 Consumer Protocol이 도입되어 일부 동작이 달라질 수 있습니다.
...
| 출처 | 설명 |
|------|------|
| [Apache Kafka JavaDoc - KafkaProducer](https://kafka.apache.org/31/javadoc/org/apache/kafka/clients/producer/KafkaProducer.html) | Producer 아키텍처, 배치 메커니즘, RecordAccumulator, 버퍼 관리 |
| [A Guide To The Kafka Protocol - Message Format](https://cwiki.apache.org/confluence/display/KAFKA/A+Guide+To+The+Kafka+Protocol#AGuideToTheKafkaProtocol-Messages) | Message Format v2, RecordBatch 바이너리 구조 |
| [A Guide To The Kafka Protocol - Produce API](https://cwiki.apache.org/confluence/display/KAFKA/A+Guide+To+The+Kafka+Protocol#AGuideToTheKafkaProtocol-Produce) | ProduceRequest/Response 프로토콜, RequiredAcks 정책 |
| [Idempotent Producer Design](https://cwiki.apache.org/confluence/display/KAFKA/Idempotent+Producer) | ProducerId, Sequence Number, 중복 제거 메커니즘 |
| [Transactional Messaging in Kafka](https://cwiki.apache.org/confluence/display/KAFKA/Transactional+Messaging+in+Kafka) | Transaction 설계, InitPhase/SendPhase/EndPhase 흐름 |
